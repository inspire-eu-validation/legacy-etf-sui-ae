...
public class SuiTestCaseDependencyDecorator implements TestCaseDependency, Comparable<SuiTestCaseDependencyDecorator> {
    ...
    /*
     * public static Collection<TestCaseDependency> getDependencies(final TestCase testCase) { final
     * List<TestCaseDependencyTestStepDef> deps = new ArrayList<>(); for (final TestStep testStep :
     * testCase.getTestStepList()) { if(testStep instanceof TestCaseDependencyTestStepDef) {
     * deps.add((TestCaseDependencyTestStepDef) testStep); } } if (!deps.isEmpty()) { return deps.stream().map(step -> new
     * SuiTestCaseDependencyDecorator(step.getTargetTestCase())).collect( Collectors.toCollection(TreeSet::new)); } return
     * null; }
     */
